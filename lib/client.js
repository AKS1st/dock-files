		...
		*/
			...
			return new Promise((resolve, reject) => {
				...
				xhr.onload = () => {
					try {
						...
						if (json.ok !== true || json.value === void 0) throw new Error(json.error?.message ?? "upload failed");
						resolve(json.value);
					} catch (cause) {
						reject(cause);
					}
		...
		function ExplorerView(props) {
			...
			*  read denied). */
			...
			const uploadingRef = (0, react.useRef)(false);
			...
			*/
						...
						const prepared = await Promise.all(items.map(async (item) => {
							...
							return {
								body: JSON.stringify({
									...
									parent: dest,
									name: item.name,
									...item.mime !== void 0 ? { mime: item.mime } : {},
									data: dataUrl.slice(dataUrl.indexOf(",") + 1)
								}),
								method: item.mime !== void 0 ? "saveImage" : "upload"
							};
						}));
			...
			const removePath = (path) => {
				setMenu(null);
				confirmDialog(`确定删除 "${baseNameOf$1(path)}"？此操作不可恢复。`, () => {
					(async () => {
						try {
			...
			*/
			...
			const buildMenuItems = () => {
				...
				const pasteImageItem = (dest) => imageProbe === "has" ? [menuItem("paste-image", imageIcon(13), "粘贴图片", () => pasteImageInto(dest))] : [];
				if (target.kind === "empty") {
					if (root === null) return [];
					const label = clipboard === null ? "粘贴" : `粘贴 ${baseNameOf$1(clipboard.path)}`;
					return [
				...
				if (target.kind === "dir") {
					const pasteLabel = clipboard === null ? "粘贴" : `粘贴 ${baseNameOf$1(clipboard.path)}`;
					return [
			...
			const dialogEl = dialog === null ? null : (0, react.createElement)(react.Fragment, null, (0, react.createElement)("div", {
			...
			}, dialog.confirmLabel ?? "确定")] : [(0, react.createElement)("button", {
				key: "ok",
			...
			return (0, react.createElement)("div", {
				...
				title: "折叠全部",
				onClick: collapseAll
			}, (0, react.createElement)("span", { className: "df-icon-stack" }, chevronUpIcon(10), chevronUpIcon(10)))), (0, react.createElement)("div", {
			...
			}, "空目录")] : rows), ...uploading ? [(0, react.createElement)("div", {
				key: "progress",
