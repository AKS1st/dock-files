...
- **File-domain service**: provides `ctx.files` (`open` / `registerFileViewer`); other plugins can register their own viewers (`exts` extension match or `default` fallback) and attach an `icon` (tint color + optional custom SVG glyph) that the explorer renders per extension, falling back to the built-in palette for unregistered types.
